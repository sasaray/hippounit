...
class ZScore_APPropagationAxon_BasketCell(Score):
    """
    ...
    def __init__(self, score, related_data={}):
        ...
        if not isinstance(score, Exception) and not isinstance(score, float):
            ...
            super(ZScore_APPropagationAxon_BasketCell,self).__init__(score, related_data=related_data)
    ...
    def compute(cls, observation, prediction):
        ...
        for key, value in observation['axon'].items():
            errors['axon'].update({int(key):{}})
            for k, v in value.items():
                p_value = prediction['axon'][int(key)][k]['mean'] 
                o_mean = v['mean'] 
                o_std = v['std']
                ...
                try:
                   ...
                   error = assert_dimensionless(error)
                    ...
                    error = e
                errors['axon'][int(key)].update({k : error})
                feature_errors=numpy.append(feature_errors, error)
